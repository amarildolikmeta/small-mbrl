...
class MBRLLoop:
    def __init__(self,
        ...
        timestamp = time.time()
        self.returns_dir = data_dir + "/" + str(timestamp)
        if not os.path.exists(self.returns_dir):
            os.makedirs(self.returns_dir)
        args_dict = {
            'ep': 0,
            'av-V-model-pi': 0,
            'av-V-env-pi': 0,
        ...
        for ep in range(args.num_eps):
            ...
            while not done:
                ...
                qf[state, action] = qf[state, action] + \
                                    args.train_type.alpha * (
                                                reward + discount * np.max(qf[next_state]) - qf[state, action])
                state = next_state
    ...
    def train_conservative_agent(self, args, p_params_baseline, baseline_true_perf):
        ...
        print(
            f'train_step: {train_step}, budget: {budget + best_cvar}, played_baseline?: {played_baseline}, budget_limit: {self.curr_budget_limit:.3f}')
    ...
    def train_constrained_agent(self, args, p_params_baseline, baseline_true_perf):
        ...
        return best_vf_model, best_cvar, highest_objective, grad_norm, best_return_distribution, budget
    ...
    def train_psrl_agent(self, args):
        ...
        for train_step in range(args.train_type.mid_train_steps):
            ...
            print(
                f'Train_step: {train_step}, Model Value fn: {vf_model:.3f}, grad_norm: {grad_norm:.3f}, cvar: {cvar_alpha:.3f}')
    ...
    def training_then_sample(self, args):
        ...
        for p_step in range(100):
            vf_model, v_alpha_quantile, cvar_alpha, return_distribution = self.agent.grad_step(self.train_type,
                                                                                               **self.args)
