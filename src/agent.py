...
class Agent:
    ...
    def value_iteration(self, r_matrix: np.ndarray, p_matrix: np.ndarray, epsilon=1e-5):
        ...
        while delta > epsilon:
            ...
            for s in range(self.nState):
                ...
                delta = max(delta, np.abs(prev_v - V[s]))
            i += 1
    ...
    def pg(self,
        ...
        V_p_pi, V_p_pi_grad, var_alpha, cvar_alpha, samples_taken = self.posterior_sampling(p_params, num_samples_plan,
    ...
    def psrl(self,
        ...
        while i < 200:
            if (i + 1) % 1000 == 0:
                self.policy_lr /= 10.
            V_p_pi, V_p_pi_grad = jax.lax.stop_gradient(jax.value_and_grad(policy_performance, 2)(R_samp, P_samp,
                                                                                                  p_params,
        ...
        self.policy.update_params(p_params)
        V_p_pi, _, var_alpha, cvar_alpha, samples_taken = self.posterior_sampling(p_params, num_samples_plan,
                                                                                  risk_threshold, R_j, P_j)
    ...
    def pg_CE(self,
        ...
        V_p_pi, V_p_pi_grad = jax.value_and_grad(policy_performance, 2)(R_CE, P_CE, p_params, self.initial_distribution,
        ...
        _, _, v_alpha_quantile, cvar_alpha, samples_taken = self.posterior_sampling(p_params, num_samples_plan,
                                                                                    risk_threshold, R_j=R_j, P_j=P_j)
    ...
    def posterior_sampling(self, p_params, num_samples_plan, risk_threshold, R_j=None, P_j=None):
        ...
        if sample_until:
            while continue_sampling:
                ...
                r = mode
                s = mode
                prob_r_s_x = lambda r_x, s_x: np.sum(np.array([comb(L_pi, i)* risk_threshold**i *
                                                               (1-risk_threshold)**(L_pi - i) for i in range(r_x, s_x)]))
                prob_r_s = prob_r_s_x(r, s)
                i = 0
                # if both have reached the ends of the array, we have the largest interval and shouldn't keep looping
                while (prob_r_s <= 1 - significance_level) and ((s != L_pi - 1) and (r != 0)):
                    if (r > 0) and (i % 2 == 0):
                        r = r - 1
    ...
    def optimistic_only(self,
        ...
        '''
        p_params = self.policy.get_params()
        U_pi, U_pi_grads, var_alpha, cvar_alpha, samples_taken = self.posterior_sampling(p_params, num_samples_plan,
                                                                                         risk_threshold, R_j=R_j,
                                                                                         P_j=P_j)
        av_vpi = np.mean(U_pi)
    ...
    def grad_risk_evaluation(self,
        ...
        significance_level = 0.1
        p_params = self.policy.get_params()
        cvar, grad_cvar = jax.value_and_grad(self.risk_evaluation_for_grad, 0)(p_params, eps_rel, significance_level,
                                                                               risk_threshold, num_samples_plan)
    ...
    def risk_evaluation_for_grad(self, p_params, eps_rel, alpha, q, num_samples):
        ...
        while continue_sampling:
            ...
            prob_r_s = prob_r_s_x(r, s)
            i = 0
            # if both have reached the ends of the array, then we have the largest interval and shouldn't keep looping
            while (prob_r_s <= 1 - alpha) and ((s != L_pi - 1) and (r != 0)):
                if (r > 0) and (i % 2 == 0):
                    r = r - 1
                if (s < L_pi - 1) and (i % 2 == 1):
                    s = s + 1
    ...
    def MC2PS(self,
        ...
        for pi_idx in range(len(all_policies)):
            var_all[pi_idx], cvar_all[pi_idx] = self.risk_evaluation(all_policies[pi_idx], all_discounts[pi_idx],
                                                                     eps_rel, significance_level, risk_threshold,
                                                                     batch_size)
    ...
    #t ry taking gradient of this and compare to CVaR one! (have to use p_params instead of policy,
    # and policy_performance instead of policy_evaluation)
