...
class GridWorld(gym.Env):
    ...
    def __init__(self, shape=(7, 7), fail_prob=0.1, goal=None, start=None, gamma=0.99,
        ...
        self.rendering = None
        self.gamma = self.discount = gamma
        ...
        self.mu = self.initial_distribution = mu
        self.P = p
        self.R = np.sum(self.P * r, axis=2)
        self.n_states = self.nState = self.W * self.H  # Number of states
        self.n_actions = self.nAction = 4  # Number of actions
        # initialize state
        self.reset()
    ...
    def calculate_mdp(self):
        ...
        return P0, P.transpose([1, 0, 2]), R.transpose([1, 0, 2])