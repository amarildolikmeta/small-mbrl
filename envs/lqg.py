...
class LQG(gym.Env):
    ...
    def __init__(self, n=2, horizon=100, gamma=0.99, max_state=10., max_action=8., sigma_noise=0.1,
                 action_scales=[0.2, 0.05], a=None,b=None, rotation=False, theta=np.pi/6, shift=0.5, norm_reward=False):
        ...
        x = np.ones(n) * max_state
        u = np.ones(n) * max_action
        self.min_reward = -np.dot(x.T, np.dot(self.Q, x)) + np.dot(u.T, np.dot(self.R, u))
        self.max_reward = 0
...
class LQGSimple(gym.Env):
    ...
    def __init__(self, n=2, horizon=100, gamma=0.99, max_state=10., max_action=8., sigma_noise=0.1,
                 A=None, B=None, R=None, Q=None, norm_reward=False):
        self.horizon = horizon
        self.gamma = gamma
        self.n = n
        self.max_pos = max_state
    ...
    def terminal_reward(self):
        if self.norm_reward:
             -self.min_reward / (self.max_reward - self.min_reward)
        else:
            return 0
